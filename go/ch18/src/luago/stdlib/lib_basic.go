...
func baseSelect(ls LuaState) int {
	n := int64(ls.GetTop())                                    // 参数个数
	if ls.Type(1) == LUA_TSTRING && ls.CheckString(1) == "#" { // select("#", ...)
		ls.PushInteger(n - 1) // 返回参数个数
		return 1
	} else {
		i := ls.CheckInteger(1) // 确保第一个参数是整数
		if i < 0 {
			i = n + i
		} else if i > n {
			i = n
		}
		ls.ArgCheck(1 <= i, 1, "index out of range") // 确保i在[1, n]之间
		return int(n - i)                            // 返回从i开始的所有参数
	}
}
